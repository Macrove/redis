...
int send_req(int fd, uint8_t *data, size_t len){
    ...
    buf_append(req, (uint8_t *)data, len);
...
int read_res(int fd){
    ...
    res.resize(4);
    int rv = read_full(fd, res.data(), 4);
    errno = 0;
    if(rv<0){
        if (errno == 0) {
            msg("EOF");
...
int main(){
    ...
    int rv = connect(fd, (struct sockaddr *)&server_addr, addrlen);
    if (rv) {
        die("Connect failed");
    }
